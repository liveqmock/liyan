...
public class EntityOperation<T> {
    ...
    String getTableName(Class<?> entityClass) {
        ...
        if (table != null && !"".equals(table.name())) name = table.name();
        return name;
    }
    ...
    String findIdProperty(Map<String, Method> getters) {
        ...
        for (String property : getters.keySet()) {
            ...
            if (id != null) {
                if (idProperty != null) throw new DbException("Duplicate @Id detected.");
                idProperty = property;
            }
        }
        if (idProperty == null) throw new DAOException("Missing @Id.");
        return idProperty;
    }
    ...
    Map<String, PropertyMapping> getPropertyMappings(Map<String, Method> getters, Map<String, Method> setters) {
        ...
        for (String property : getters.keySet()) {
            Method getter = getters.get(property);
            if (getter.isAnnotationPresent(Transient.class)) continue;
            Method setter = setters.get(property);
            if (setter == null) throw new DbException("Missing setter while getter " + getter.getName() + " found.");
    ...
    public SQLOperation insertEntity(Object entity) throws Exception {
        ...
        Map<String, String> attributeColumnMapping = new HashMap<String, String>();
        for (MdField mdField : mdFields) {
            if (mdField.getIsExtend().equals(FrameworkConstants.COMM_Y)) {
                attributeColumnMapping.put(mdField.getEntityAttriName(), mdField.getFieldName().toUpperCase());
            }
        }
        ...
        if (SQL_INSERT == null) {
            ...
            for (Map.Entry<String, String> entry : attributeColumnMapping.entrySet()) {
                insertableProperties.add(entry.getKey());
        ...
        for (int i = 0; i < INSERT_PROPERTIES.length; i++) {
            if (null!=mappings.get(INSERT_PROPERTIES[i])&&mappings.get(INSERT_PROPERTIES[i]).id) {
                params[i] = UUID.randomUUID() + "";
            } else {
                if (attributeColumnMapping.containsKey(INSERT_PROPERTIES[i])) {
                    params[i] = EntityUtils.getValue(entity, INSERT_PROPERTIES[i]);
                } else {
    ...
    public SQLOperation updateEntity(Object entity) throws Exception {
        IMdFieldService mdFieldService = ContextHolder.getSpringBean("mdFieldService");
        List<MdField> mdFields = mdFieldService.findLastVersionMdFieldsByTableName(tableName.toUpperCase());
        Map<String, String> attributeColumnMapping = new HashMap<String, String>();
        for (MdField mdField : mdFields) {
        ...
        if (SQL_UPDATE_BY_ID == null) {
            ...
            for (Map.Entry<String, String> entry : attributeColumnMapping.entrySet()) {
                updatableProperties.add(entry.getKey());
                sb.append(entry.getValue()).append("=?,");
            }
        ...
        for (int i = 0; i < UPDATE_PROPERTIES.length; i++) {
            if (attributeColumnMapping.containsKey(UPDATE_PROPERTIES[i])) {
                params[i] = EntityUtils.getValue(entity, UPDATE_PROPERTIES[i]);
            } else {
