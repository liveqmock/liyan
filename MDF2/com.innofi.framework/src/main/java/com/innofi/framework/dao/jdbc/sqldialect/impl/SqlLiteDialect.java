 ...
 * SqlLite数据库实现方言类
 */
public class SqlLiteDialect extends AbstractDbDialect {
    ...
    public String getTableRenameSql(String schema,String tableName, String newTableName) {
        if(StringUtil.hasText(schema)){
            return "RENAME TABLE " + schema+"."+tableName + " TO " + schema+"."+newTableName;
        }
    ...
    public String getDropSequenceSql(String schema, String sequenceName) {
        //--todo 无实现
        throw new FrameworkJdbcRuntimeException("此数据库暂不支持此操作！");
    }
